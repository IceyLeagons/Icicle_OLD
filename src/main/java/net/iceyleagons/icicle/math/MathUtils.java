...
public final strictfp class MathUtils {
     ...
     * (Different from {@link Location#toVector()}
     *
    ...
    public static Vector getVector(Location from, Location to) {
        Asserts.notNull(from, "Location \"from\" must not be null!");
        Asserts.notNull(from, "Location \"to\" must not be null!");
    ...
    public static double normalize(double value, double min, double max) {
        Asserts.isTrue(min != max, "Minimum and Maximum must be different!");
    ...
    public static int roundToInt(double value, int precision, RoundingMode mode) {
        return BigDecimal.valueOf(value).round(new MathContext(precision, mode)).intValue();
    }
     ...
     */
    public static int random(@Nullable Integer lowerBound, @Nullable Integer higherBound) {
        ThreadLocalRandom random = ThreadLocalRandom.current();

        if (lowerBound != null && higherBound != null) {
            return random.nextInt(higherBound - lowerBound) + lowerBound;
        } else {
            return lowerBound != null ? random.nextInt(MAX_VALUE - lowerBound) + lowerBound : random.nextInt();
        }
    }
    ...
    /**
     * Return the suffix for the number. Ex it's the 1st, 2nd, 3rd etc.
     *
     * @param value the number to generated the suffix for
     * @param appendNumber if true the number will get added in front of the suffix
     * @return the suffix. (Does not contain the number, unless specified!)
     */
    public static String getSuffix(int value, boolean appendNumber) {
        int hunRem = value % 100;
        int tenRem = value % 10;

        final String thString = appendNumber ? String.valueOf(value) : "" + "th";
        if (hunRem - tenRem == 10) return thString;

        switch (tenRem) {
            case 1:
                return appendNumber ? String.valueOf(value) : "" + "st";
            case 2:
                return appendNumber ? String.valueOf(value) : "" + "nd";
            case 3:
                return appendNumber ? String.valueOf(value) : "" + "rd";
            default:
                return thString;
        }
    }
