...
import net.iceyleagons.icicle.common.annotations.Bean;
...
public class ClassScanningHandler {
        ...
        classes.forEach(clazz -> {
            try {
                if (clazz.getSuperclass().equals(AbstractAnnotationHandler.class) && !clazz.isAnnotation() && !clazz.isInterface()) {
                    Constructor<?> constructor = net.iceyleagons.icicle.common.reflect.Reflections.getConstructor(clazz, true);
                    if (constructor != null) {
    ...
    private void loadAutowiringHandlers() {
        Set<Class<?>> classes = rootPackage.getTypesAnnotatedWith(net.iceyleagons.icicle.common.annotations.autowiring.AutowiringHandler.class);
        classes.forEach(clazz -> {
            try {
                if (clazz.getSuperclass().equals(AbstractAutowiringHandler.class) && !clazz.isAnnotation() && !clazz.isInterface()) {
                    ...
                    if (constructor != null) {
                        ...
                        abstractAutowiringHandler.setAnnotation(clazz.getAnnotation(net.iceyleagons.icicle.common.annotations.autowiring.AutowiringHandler.class));
