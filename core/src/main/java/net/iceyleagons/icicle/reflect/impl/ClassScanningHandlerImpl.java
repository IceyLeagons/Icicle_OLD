...
@Getter
public class ClassScanningHandlerImpl implements ClassScanningHandler {

    private final RegisteredPlugin registeredPlugin;
    private final AutowiringHandlerImpl autowiringHandler;
    private final org.reflections.Reflections rootPackage;
    private final Logger logger = Logger.getLogger(ClassScanningHandlerImpl.class.getName());
    ...
    public ClassScanningHandlerImpl(RegisteredPlugin registeredPlugin) {
        this.registeredPlugin = registeredPlugin;
        this.registeredPlugin.setClassScanningHandler(this);

        this.autowiringHandler = new AutowiringHandlerImpl(this);
        this.registeredPlugin.setAutowiringHandler(this.autowiringHandler);

        this.rootPackage = new org.reflections.Reflections(registeredPlugin.getRootPackageName()).merge(new org.reflections.Reflections("net/iceyleagons/icicle"));
        initialize();
    }
    ...
    private void loadAutowiringHandlers() {
        Set<Class<?>> classes = rootPackage.getTypesAnnotatedWith(AutowiringHandler.class);
        classes.forEach(clazz -> {
            try {
                if (clazz.getSuperclass().equals(AbstractAutowiringHandler.class) && !clazz.isAnnotation() && !clazz.isInterface()) {
                    ...
                    if (constructor != null) {
                        ...
                        abstractAutowiringHandler.setAnnotation(clazz.getAnnotation(AutowiringHandler.class));
